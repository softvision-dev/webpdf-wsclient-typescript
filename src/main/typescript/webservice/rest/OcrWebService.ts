...
import {Billing, Ocr, OcrOperation, OcrOperationInterface, PdfPassword, Settings} from "../../generated-sources";
 ...
 */
export class OcrWebService<T_REST_DOCUMENT extends RestDocument>
	extends RestWebService<OcrOperation, Ocr, T_REST_DOCUMENT> {
	/**
	 * Creates a {@link OcrWebService} for the given {@link RestSession}
	 *
	 * @param session The {@link RestSession} a {@link OcrWebService} shall be created for.
	 */
	public constructor(session: RestSession<T_REST_DOCUMENT>) {
		super(session, WebServiceTypes.OCR);
	}

	/**
	 * Returns the {@link OcrWebService} specific {@link Ocr}, which allows setting parameters
	 * for the webservice execution.
	 *
	 * @return The {@link Ocr} operation parameters.
	 */
	public getOperationParameters(): Ocr {
		return this.getOperationData().ocr;
	}

	/**
	 * Sets the {@link OcrWebService} specific {@link Ocr} element, which allows setting
	...
	public setOperationParameters(operation: Ocr | undefined) {
		...
		this.getOperationData().ocr = operation;
	}
	 ...
	 *
	 * @return The prepared {@link OcrOperation}.
	 */
	protected initOperation(): OcrOperation {
		return OcrOperation.fromJson({
